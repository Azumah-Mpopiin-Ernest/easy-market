...
export default function MainComponent() {
  ...
  function handleCheck(id) {
    if (selectedItem.includes(id)) {
      const newSelectedItems = selectedItem.filter((item) => item !== id);
  ...
  const listEls = list.map((item) => (
    <div
      className="w-full flex justify-between items-center max-w-3xl"
      key={item.id}
    >
      <div className="flex justify-between items-center bg-white rounded-3xl p-3 gap-1">
        <div className="flex items-center w-1/2 gap-2">
          <div className="flex items-center gap-1">
            <p className=" text-md tracking-tighter leading-none">
              {item.name}
            </p>
            <span>—</span>
            <input
        ...
        {selectedItem.includes(item.id) ? (
          <span
            ...
            className="material-symbols-outlined  cursor-pointer text-green-600 "
          >
            check_circle
          </span>
        ) : (
  ...
  function downloadList() {
    ...
    list.forEach((item) => {
      ...
      content += `${isChecked} ${item.name} — ${item.price} x ${
        item.quantity
      } = ${item.price * item.quantity}\n`;
    });
  ...
  return (
    <>
      <main
        className="flex flex-col gap-5 items-center px-3 "
      ...
      >
        {alertMesg && (
          <div className="fixed top-60 w-full flex justify-center z-50">
            <p
              ...
              style={{
                backgroundColor:
                  alertType === "success" ? "#047857" : "#F87171",
              }}
            >
              {alertMesg}
            </p>
        ...
        <div className="bg-blue-600 w-full rounded flex justify-between h-10 items-center py-8 px-5 max-w-3xl">
          ...
          <p className="flex gap-2 items-center">
            <span className="text-lg text-yellow-300">{list.length}</span>
            <span className="text-white">
              Item{list.length === 0 || list.length > 1 ? "s" : ""}
            </span>
          </p>
          <button
            className="bg-yellow-300 px-2 rounded hover:bg-yellow-200"
            onClick={() => {
              if (list.length > 0) {
                saveBasket();
              } else {
                showAlert("Add items to save!", "error");
                return;
              }
            }}
          >
        ...
        {list.length > 0 ? (
          ...
          <p className="mb-20">The Basket is...Empty!</p>
        )}
        ...
        {list.length > 0 && (
          <>
                  ...
                  <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/30">
                    <div className="flex flex-col items-center justify-center gap-6 bg-blue-600 rounded-lg w-80 max-w-[90%] h-40 p-4 animate-fadeIn">
                      <h3 className="text-yellow-100">
                        Are you sure you want to empty Basket?
                      </h3>
            ...
            <div className="flex items-center justify-between w-full px-5 mb-20 max-w-3xl">
              <button
                className="bg-yellow-500 rounded-xl px-2 cursor-pointer hover:bg-yellow-200"
      ...
      {showInstallPopup && !isInStandalone && (
        <div
          ...
          style={{ fontFamily: "Poppins, sans-serif" }}
        >
          <div className="bg-white border shadow-xl rounded-xl p-4 w-80 max-w-[90%]">
      ...
      <footer className="bg-yellow-500 text-white flex flex-col h-40 justify-center px-5">
        <div className="flex justify-between items-center mb-5">
          ...
          <span className="mr-4 cursor-pointer">Enjoy the market!</span>
        </div>
